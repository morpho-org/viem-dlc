...
describe("LruStore", () => {
  ...
  it("evicts oldest entries when byte limit is exceeded", async () => {
    ...
    await store.set("a", "1"); // 1 byte
    await store.set("b", "2"); // 1 byte, total 2
    await store.set("c", "3"); // 1 byte, would be 3, evicts 'a', total 2
  ...
  it("updates access order on get", async () => {
    ...
    await store.set("c", "3"); // 1 byte, evicts 'b'
  ...
  it("evicts multiple entries if needed for a large value", async () => {
    ...
    await store.set("a", "11"); // 2 bytes
    await store.set("b", "22"); // 2 bytes
    await store.set("c", "33"); // 2 bytes, total 6

    // New value is 8 bytes, total would be 14, must evict 'a' and 'b' to fit (need <= 2 bytes)
    await store.set("x", "12345678"); // 8 bytes

    expect(await store.get("a")).toBeNull();
    expect(await store.get("b")).toBeNull();
    expect(await store.get("c")).toBe("33");
    expect(await store.get("x")).toBe("12345678");
  });
  ...
  it("correctly tracks bytes when updating with different sized values", async () => {
    const store = new LruStore(10);
    await store.set("a", "123456789"); // 9 bytes
    await store.set("a", "1"); // now 1 byte

    // Should have room for more entries now
    await store.set("b", "12345678"); // 8 bytes, total 9
    expect(await store.get("a")).toBe("1");
    expect(await store.get("b")).toBe("12345678");
  });
  ...
  it("warns and skips values that exceed maxBytes", async () => {
    ...
    await store.set("a", "1"); // 1 byte, fits
    await store.set("big", "123456"); // 6 bytes, exceeds 5

    expect(warnSpy).toHaveBeenCalledWith("[LruStore] Value exceeds maxBytes (6 > 5), skipping");
    expect(await store.get("a")).toBe("1"); // original entry still there
  ...
  it("does not evict existing entries when new value exceeds maxBytes", async () => {
    ...
    await store.set("a", "12"); // 2 bytes
    await store.set("b", "34"); // 2 bytes, total 4
    await store.set("huge", "123456"); // 6 bytes, exceeds 5

    // Existing entries should not be evicted
    expect(await store.get("a")).toBe("12");
    expect(await store.get("b")).toBe("34");
  ...
  it("evicts other entries when updating existing key with larger value", async () => {
    const store = new LruStore(10);
    await store.set("a", "12345"); // 5 bytes
    await store.set("b", "12345"); // 5 bytes, total 10

    // Update 'a' with larger value - should evict 'b' to make room
    await store.set("a", "1234567890"); // 10 bytes

    expect(await store.get("a")).toBe("1234567890");
