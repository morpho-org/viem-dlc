...
describe("isRpcError", () => {
  it("returns true for RpcError instances", () => {
    const error = new RpcError(new Error("test"), {
      code: -32000,
      shortMessage: "test",
    });
  ...
  it("returns true for Error with code and message properties", () => {
    const error = Object.assign(new Error("test message"), { code: -32000 });
    expect(isRpcError(error)).toBe(true);
  });
  ...
  it("returns false for non-Error values", () => {
    ...
    expect(isRpcError({ code: -32000, message: "test" })).toBe(false);
