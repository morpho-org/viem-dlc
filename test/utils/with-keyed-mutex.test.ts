...
import { createKeyedMutex } from "../../src/utils/with-keyed-mutex.js";

describe("createKeyedMutex", () => {
  it("returns an object with withKeyedMutex", () => {
    const { withKeyedMutex } = createKeyedMutex();
    expect(typeof withKeyedMutex).toBe("function");
  });
});
...
describe("withKeyedMutex", () => {
  it("executes function and returns result", async () => {
    const { withKeyedMutex } = createKeyedMutex();
    const result = await withKeyedMutex("key", () => Promise.resolve("hello"));
  ...
  it("propagates errors from the wrapped function", async () => {
    ...
    await expect(withKeyedMutex("key", () => Promise.reject(error))).rejects.toThrow("test error");
  });

  it("cleans up lock after completion", async () => {
    const { withKeyedMutex } = createKeyedMutex();

    await withKeyedMutex("key", () => Promise.resolve("done"));

    // No way to inspect internal map size, but subsequent operations should work
    const result = await withKeyedMutex("key", () => Promise.resolve("again"));
  ...
  it("cleans up lock after error", async () => {
    ...
    await expect(withKeyedMutex("key", () => Promise.reject(new Error("fail")))).rejects.toThrow();

    // Should still work after error
    const result = await withKeyedMutex("key", () => Promise.resolve("recovered"));
  ...
  describe("key isolation", () => {
    it("allows concurrent operations on different keys", async () => {
      ...
      const promiseA = withKeyedMutex("keyA", async () => {
        order.push("A-start");
        await sleep(50);
        order.push("A-end");
        return "A";
      ...
      const promiseB = withKeyedMutex("keyB", async () => {
        order.push("B-start");
        await sleep(20);
        order.push("B-end");
        return "B";
  ...
  describe("serialization", () => {
    it("serializes concurrent operations on the same key", async () => {
      ...
      const promise1 = withKeyedMutex("same-key", async () => {
        order.push(1);
        await sleep(30);
        order.push(11);
        return 1;
      ...
      const promise2 = withKeyedMutex("same-key", async () => {
        order.push(2);
        await sleep(10);
        order.push(22);
        return 2;
      ...
      const promise3 = withKeyedMutex("same-key", async () => {
        order.push(3);
        order.push(33);
        return 3;
      });
  ...
  describe("error handling", () => {
    it("releases lock and allows subsequent operations after error", async () => {
      ...
      await expect(withKeyedMutex("key", () => Promise.reject(new Error("first fails")))).rejects.toThrow(
        "first fails",
      );

      // Second operation should succeed
      const result = await withKeyedMutex("key", () => Promise.resolve("success"));
    ...
    it("serializes operations even when some fail", async () => {
      ...
      const promise1 = withKeyedMutex("key", async () => {
        order.push("op1-start");
        order.push("op1-end");
        return "ok1";
      });

      const promise2 = withKeyedMutex("key", async () => {
        order.push("op2-start");
        throw new Error("op2 fails");
      });

      const promise3 = withKeyedMutex("key", async () => {
  ...
  describe("edge cases", () => {
    it("handles single operation", async () => {
      const { withKeyedMutex } = createKeyedMutex();
      const result = await withKeyedMutex("key", () => Promise.resolve(42));
    ...
    it("handles rapid sequential operations", async () => {
      ...
      for (let i = 1; i <= 5; i++) {
        const result = await withKeyedMutex("key", () => Promise.resolve(i));
        results.push(result);
      }
    ...
    it("handles mixed concurrent and sequential operations across keys", async () => {
      ...
      const a1 = withKeyedMutex("keyA", async () => {
        log.push("A1");
        await sleep(20);
        return "A1";
      });
      const a2 = withKeyedMutex("keyA", async () => {
        log.push("A2");
        return "A2";
      });

      // Start operations on keyB (should run concurrently with keyA)
      const b1 = withKeyedMutex("keyB", async () => {
        log.push("B1");
        return "B1";
      });
