/** biome-ignore-all lint/suspicious/noExplicitAny: light use makes testing easier here */
  ...
  describe("probabilistic invalidation", () => {
    it("refetches when invalidation strategy returns 1", async () => {
      ...
      await handleGetLogs(
        ...
        chainId,
        { fromBlock: "0x0", toBlock: "0x270f" },
        { binSize, invalidationStrategy: alwaysInvalidate, cache },
      );
    ...
    it("passes correct context to invalidation strategy", async () => {
      ...
      cache.storage.set(
        cacheKey,
        createCachedChunk(0n, 9999n, [cachedLog], {
          fetchedAtBlock,
          fetchedAt,
        }),
      );
      ...
      await handleGetLogs(
        ...
        chainId,
        { fromBlock: "0x0", toBlock: "0x270f" },
        { binSize, invalidationStrategy, cache },
      );
  ...
  describe("edge cases", () => {
    ...
    it('resolves "earliest" block tag to 0', async () => {
      ...
      await handleGetLogs(
        requestFn,
        chainId,
        { fromBlock: "earliest", toBlock: "0x270f" },
        { binSize, invalidationStrategy: neverInvalidate, cache },
      );
