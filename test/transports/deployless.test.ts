...
import { EIP_3860_INITCODE_SIZE, monadFacts } from "../../src/chains/index.js";
...
describe("deployless", () => {
  ...
  describe("batching", () => {
    ...
    it("packs under the chain's initcode limit with no request size stated", async () => {
      const requestFn = mockPagedFn();
      const transport = createTransport(requestFn);
    ...
    it("forwards block, cleaned stateOverride, and blockOverride upstream", async () => {
      ...
      const req: EthCallRequest = {
        ...
        params: [
          ...
          "0x100" as Hex,
          { ...policySentinel(pageAbi), ...extraOverride },
          blockOverride,
        ],
      };
  ...
  it("ignores policy.cache and behaves like split-only mode", async () => {
    ...
    const withCache = await createTransport(requestFnWithCache, config).request(
      createRequest(accounts, { withCache: true }),
    );
  ...
  describe("compress=true", () => {
    ...
    it("sets the compression bit and compresses only the body", async () => {
      ...
      await transport.request(createRequest(accounts, { batch: { compress: true } }));
  ...
  describe("halve-on-error retries", () => {
    it("halves and retries when upstream rejects with a batch-size error", async () => {
      ...
      const { logger, events } = createStubLogger();
      const result = await withLogging(() => transport.request(createRequest(accounts)), { logger });
      const field = (name: string) => findDotted(events[0]!.context, "viem-dlc-deployless", `eth_call.${name}`);
    ...
    it("throws on the wrapper's out-of-gas marker instead of halving", async () => {
      ...
      await expect(transport.request(createRequest(addrs(4)))).rejects.toThrow(/ran out of gas under this node's cap/);
      expect(requestFn).toHaveBeenCalledOnce();
    });
    ...
    it("does not treat lens revert data that merely starts with the marker as out-of-gas", async () => {
      ...
      await expect(transport.request(createRequest([addr(1), addr(2)]))).rejects.toThrow("execution reverted");
    ...
    it("throws a counterfactual-deploy failure instead of halving it", async () => {
      ...
      await expect(transport.request(createRequest(addrs(4)))).rejects.toThrow(/counterfactual deploy failed/);
    ...
    it("rethrows when a single-element batch fails with a batch-size error", async () => {
      const tooLarge = Object.assign(new Error("request body too large"), { data: "0x" as Hex });
      const requestFn = vi.fn().mockRejectedValue(tooLarge);
      const transport = createTransport(requestFn);

      await expect(transport.request(createRequest([addr(1)]))).rejects.toThrow("request body too large");
      expect(requestFn).toHaveBeenCalledTimes(1);
    });
    ...
    it("does not retry on unrecognized errors", async () => {
      ...
      const { logger, events } = createStubLogger();
      const error = await withLogging(() => transport.request(createRequest([addr(1), addr(2)])), { logger }).catch(
        (e) => e,
      );
      const field = (name: string) => findDotted(events[0]!.context, "viem-dlc-deployless", `eth_call.${name}`);
    ...
    it.each([
      ...
      await expect(transport.request(createRequest(addrs(4)))).rejects.toThrow();
    ...
    it("halves on timeout and succeeds when smaller chunks come back", async () => {
      ...
      const result = await transport.request(createRequest(accounts));
