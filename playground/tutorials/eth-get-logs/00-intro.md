...
Subtract that deployment block from the current head and you have the range you actually want: on
Base today, roughly 37 million blocks of history behind one `eth_getLogs` call.
...
*Measured September 2026. The second endpoint is a paid plan, left unnamed because the point is that
the two numbers differ, not which vendor sets which. Neither error code is standardized — providers
invent their own, which is the first reason your code can't detect a cap by reading one.*
