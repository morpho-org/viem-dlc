...
This is the part that makes hand-rolling it unpleasant. The cap isn't in any header. It varies by
provider, by plan, and by how busy the contract was in the window you asked for — the same 100,000
blocks that answer fine today return a response-size error once the contract gets popular. Any loop
you write has to discover the limit at runtime and back off when it finds one.
...
reason this transport goes underneath essentially every `eth_getLogs` call, whoever you're
talking to.
