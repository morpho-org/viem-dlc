...
dispatches between bare transports, so it has nowhere to put any of that — the per-provider state
you need is exactly the state it can't hold.

**The store is shared.** Branch A fetching half a range before it dies is not wasted: those bins are
already in the store, and branch B starts from what's there.
...
next provider, and trying it there only delays the error you need to see.
