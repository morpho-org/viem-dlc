...
These describe the lens and the provider, not this request, so take them over a representative
window rather than one run — costs depend on which elements share a frame, and grouping related
elements warms storage they share. A figure that's stale costs one round trip and never a result.
...
and they all fit in one, so the second number is just 60 — a fact about your request, not about what
bytes allowed. Raise the vault count until `nominal_batches` climbs above 1 before reading it.

One caveat on `gas_limit_observed`: it's the cap the provider granted at that moment. A `gasLimit`
above it is a cap that has since been lowered, which is worth alerting on rather than pasting.
