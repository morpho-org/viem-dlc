...
One rule: **the library emits nothing until you ask.** There is no global logger, no ambient
config, and no output on any path you haven't wrapped. Wrap a call in `withLogging` and every
outermost transport call inside that scope produces exactly one event when it concludes.
