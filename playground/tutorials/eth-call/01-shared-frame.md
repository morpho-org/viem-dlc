viem splits a `multicall` into batches by **calldata bytes**. The `batchSize` option sets the budget,
and it defaults to 1,024 bytes — about fifteen of these reads. Run the step below as it stands and
...
revert, so you lose results and can't tell why. That alone is a reason to reach for `batchSize`.
...
a batch draws on one shared frame. An expensive element takes its share first, and EIP-150 leaves the
outer frame only 1/64 of what it forwarded — enough for a handful of ordinary reads, not for the 119
waiting behind it. When that runs out, the whole `eth_call` exceeds the node's cap, the node returns
an error, and you get nothing rather than the 119 that were fine.
...
failure local, predictable, or recoverable.

So the safe `batchSize` isn't a property of this call site. It's a property of the most
gas-intensive multicall anywhere in your application, applied everywhere. You can set it per call
site instead, but then viem can no longer merge concurrent multicalls from different parts of your
app into one request, because they no longer agree on the budget — and you've given up the automatic
batching you raised `batchSize` to get.
