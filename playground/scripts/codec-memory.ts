 ...
 *
 * Run both sides and compare — the playground config aliases `zlib` to the shim, a bare run does
 * not, so the same code runs against gzip-over-CompressionStream and real Node zstd:
 *
 *   NODE_OPTIONS=--expose-gc LABEL="node zstd" pnpm exec vite-node playground/scripts/codec-memory.ts
