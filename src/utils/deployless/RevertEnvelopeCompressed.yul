/*
 * Outer constructor wrapper for deployless eth_call with FLZ wire compression (Yul source).
 *
 ...
 *   JS side:        decompresses returndata after extracting from revert
 *
 * On lens success:  revert(OK_SENTINEL || flzCompress(returndata))
 ...
 *   [htEnd, htEnd+4)              OK_SENTINEL slot
 *   [htEnd+4, ...)                flzCompress(returndata) output
 *
...
object "RevertEnvelopeCompressed" {
    code {
        ...
        if ok {
            ...
            // Write OK_SENTINEL at htBase+0x8000, compressed output at htBase+0x8000+4.
            let sentinelPtr := add(htBase, 0x8000)
            let outDataPtr  := add(sentinelPtr, 0x04)
            let cmpLen := flzCompress(rdPtr, rdLen, htBase, outDataPtr)

            // Write 4-byte sentinel individually to avoid clobbering compressed output.
            // OK_SENTINEL = bytes4(keccak256("ViemDlcOk()")) = 0x1580d19d
            mstore8(sentinelPtr,            0x15)
            mstore8(add(sentinelPtr, 0x01), 0x80)
            mstore8(add(sentinelPtr, 0x02), 0xd1)
            mstore8(add(sentinelPtr, 0x03), 0x9d)
            revert(sentinelPtr, add(0x04, cmpLen))
        }
