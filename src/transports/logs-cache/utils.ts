...
import type { EthGetLogsFilter } from "../../types.js";
import { cyrb64Hash } from "../../utils/hash.js";

/** Separator between filter part and block range part of cache key */
export const CACHE_KEY_SEPARATOR = "+";

function normalizeAddress(address: EthGetLogsFilter["address"]): string {
  if (!address) return "*";
...
function normalizeTopics(topics: EthGetLogsFilter["topics"]): string {
  if (!topics) return "*";
