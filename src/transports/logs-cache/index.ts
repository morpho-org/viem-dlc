...
import type { EIP1193PublicRequestFn } from "../../types.js";
...
export function logsCache(
  ...
  return (params) => {
    ...
    const request = (args: EIP1193Parameters<PublicRpcSchema>) => {
      if (args.method !== "eth_getLogs") {
        return transport.request(args);
      }

      // TODO: (@haydenshively future-work) `handleGetLogs` could respect `options`
      return handleGetLogs(transport.request, chainId, args.params[0], {
        binSize,
        invalidationStrategy,
        cache,
        cacheKeyIncludes,
