...
| [Page telemetry: every page an observation, every wave a prediction](./000016-tib-page-telemetry.md) | | The envelope reports the frame's usable budget, what it spent before its first attempt, and the sum, sum of squares and maximum of per-attempt gas on every page; later waves are packed to a prediction with a stated exceedance bound; the opening wave is packed by the same estimator from the transport's `gasLimit` and the policy's `batch.gas`, both readable off any request. |
