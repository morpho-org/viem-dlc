...
| [Paginated lenses: pagination in the envelope, with no load-bearing gas figure](./000016-tib-paginated-lenses.md) | `d812e07` | A lens is one `view` function per element; the envelope calls it once per element in its own frame, admits every attempt against the fee schedule, streams one record per adjudicated element, decompresses input as attempted, and reports per-page gas telemetry; the client packs every chunk by one prediction — from the provider's cap and the lens's stated cost for the opening wave, from measurement after — and pools the elements pages did not reach into coalesced continuations; nothing is load-bearing. Consolidates four earlier 000016 TIBs (in git history). |
| [Override-delivered envelope: the frame's gas as the only chunk bound](./000016-tib-override-delivered-envelope.md) | | `batch.envelope: "override"` delivers a chunk as a call to a fixed address whose code is the envelope, placed by `eth_call`'s state override, so the initcode cap no longer bounds a chunk; the lens is still deployed by the factory inside the call; the predicate prices intrinsic gas per delivery, moves the argument copy out of `fixed`, and gains EIP-7623's floor; an unsupporting provider is detected on the opening wave and re-fetched as initcode; chain definitions record how a chain's nodes frame an `eth_call`. |
