...
| [Outcome stream: envelope memory proportional to progress, no declared sizes](./000016-tib-outcome-stream.md) | | The response becomes one record per attempt with an O(1) exit; dynamic elements are length-prefixed on the wire; every memory expansion is admitted before it is paid; the two envelopes merge into one; `maxItemBytes` / `maxResultBytes` are deleted. |
| [Streaming decompression: the compressed path without a prologue](./000016-tib-streaming-decompression.md) | | The FLZ decompressor keeps its cursors across the loop and writes into a fixed history, so a compressed chunk costs nothing until attempted; `MAX_ALLOC_BYTES` and the corpse bisection class are deleted. |
| [Page telemetry: every page an observation, every later wave a prediction](./000016-tib-page-telemetry.md) | | The envelope reports the frame's usable budget and the sum, sum of squares and maximum of per-attempt gas on every page; later waves are packed to a prediction with a stated exceedance bound; `batch.pageSizeHint` sizes the opening wave and `page_size_suggested` says what to set it to. |
| [Opening wave: the provider's cap and the lens's cost as separate numbers](./000016-tib-opening-wave.md) | | `batch.pageSizeHint` is replaced by the transport's `gasLimit` and the policy's `batch.gas`; the envelope reports what the frame spent before its first attempt; the opening wave is packed by the continuation estimator against the cap less the calldata's intrinsic gas; `fixed_gas` and `gas_limit_observed` make both figures readable off any request. |
