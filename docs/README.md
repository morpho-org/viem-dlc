...
| [Override-delivered envelope: the frame's gas as the only chunk bound](./000016-tib-override-delivered-envelope.md) | | `batch.envelope: "override"` delivers a chunk as a call to a fixed address whose code is the envelope, placed by `eth_call`'s state override, so a chunk is bounded by the frame's gas rather than the initcode cap; an unsupporting provider is detected on the opening wave and the range re-fetched as initcode. Plan: APPS-1406. |
